...
try {
    ...
    $productos_sin_preparar = $pdo->prepare("
        ...
        WHERE op.orden_id = ? 
        AND COALESCE(op.preparado, 0) = 0 
        AND COALESCE(op.cancelado, 0) = 0
    ");
    $productos_sin_preparar->execute([$orden_id]);
    $pendientes = $productos_sin_preparar->fetchColumn();
    
    if ($pendientes > 0) {
        throw new Exception("No se puede cerrar la orden. Hay {$pendientes} producto(s) sin preparar. Por favor, complete la preparación o cancele los productos pendientes antes de cerrar.");
    }
    ...
    if ($impresion_automatica && !empty($nombre_impresora)) {
        try {
            ...
            // Obtener productos preparados (no cancelados)
            $stmt = $pdo->prepare("
                SELECT op.*, p.nombre, p.precio 
                FROM orden_productos op 
                JOIN productos p ON op.producto_id = p.id 
                WHERE op.orden_id = ? AND op.preparado = 1 AND COALESCE(op.cancelado, 0) = 0
            ");
            $stmt->execute([$orden_id]);
            ...
            $empresaNombre = $empresaRow ? $empresaRow['valor'] : 'Kalli Jaguar';
