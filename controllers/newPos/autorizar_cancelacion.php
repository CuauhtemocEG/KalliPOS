...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT op.id, op.cantidad, op.preparado, op.cancelado, p.nombre
    ...
    error_log("DEBUG Autorización: Producto encontrado, cantidad actual = " . $producto_orden['cantidad']);
    ...
    $cantidad_cancelar = min($cancelacion['cantidad_solicitada'], $producto_orden['cantidad']);
    $cantidad_restante = $producto_orden['cantidad'] - $cantidad_cancelar;
    
    if ($cantidad_restante <= 0) {
        // Cancelar completamente el producto
        $stmt = $pdo->prepare("
            UPDATE orden_productos 
            SET cancelado = 1, cancelado_por_usuario_id = ? 
            WHERE id = ?
        ");
        $stmt->execute([$admin_user_id, $producto_orden['id']]);
        error_log("DEBUG Autorización: Producto cancelado completamente");
    } else {
        // Cancelación parcial: reducir cantidad y crear registro cancelado
        $stmt = $pdo->prepare("
            UPDATE orden_productos 
            SET cantidad = ? 
            WHERE id = ?
        ");
        $stmt->execute([$cantidad_restante, $producto_orden['id']]);
        
        // Crear nuevo registro para la cantidad cancelada
        $stmt = $pdo->prepare("
            INSERT INTO orden_productos (orden_id, producto_id, cantidad, cancelado, preparado, cancelado_por_usuario_id) 
            VALUES (?, ?, ?, 1, 0, ?)
        ");
        $stmt->execute([$cancelacion['orden_id'], $cancelacion['producto_id'], $cantidad_cancelar, $admin_user_id]);
        error_log("DEBUG Autorización: Cancelación parcial - reducida cantidad a $cantidad_restante, cancelados $cantidad_cancelar");
    }
