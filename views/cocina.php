...
<div class="text-center mb-12">
  <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-orange-500 to-red-600 rounded-3xl mb-6 shadow-2xl">
    <i class="bi bi-fire text-white text-3xl"></i>
  </div>
  <h1 class="text-4xl md:text-5xl font-bold font-display gradient-text mb-4">
    Vista de Cocina
  </h1>
  <p class="text-xl text-gray-400 max-w-2xl mx-auto">
    Gestiona y controla todas las órdenes de comida en preparación
  </p>
</div>
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center space-x-4">
        <div class="flex items-center space-x-2">
          ...
          <span class="text-green-400 text-sm font-medium">En línea</span>
        </div>
        <button onclick="cargarCocina()" 
                class="px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold rounded-xl transition-all duration-200 transform hover:scale-105">
...
function cargarCocina() {
  ...
  document.getElementById('cocina-content').innerHTML = `
    <div class="bg-dark-700/30 backdrop-blur-xl rounded-2xl border border-dark-600/50 p-12 text-center">
      ...
      <h3 class="text-xl font-semibold text-white mb-2">Actualizando Vista de Cocina</h3>
      <p class="text-gray-400">Obteniendo órdenes pendientes...</p>
    </div>
  `;
        ...
        for (const nombreMesa in mesas) {
          html += `
            <div class="mb-8 bg-dark-700/30 backdrop-blur-xl rounded-2xl border border-dark-600/50 overflow-hidden shadow-xl">
              <div class="bg-gradient-to-r from-orange-500/20 to-red-600/20 p-6 border-b border-dark-600/50">
                <div class="flex items-center space-x-3">
              ...
              <div class="overflow-x-auto">
                <table class="w-full">
                  <thead class="bg-dark-600/50">
                    <tr>
                      <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300">Producto</th>
          ...
          mesas[nombreMesa].forEach(item => {
            html += `
              <tr class="hover:bg-dark-600/30 transition-colors">
                <td class="px-6 py-4">
                  <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 bg-gradient-to-br from-blue-500/20 to-purple-600/20 rounded-lg flex items-center justify-center">
                      <i class="bi bi-egg-fried text-blue-400"></i>
                    </div>
                    <div>
                ...
                <td class="px-6 py-4">
                  ${item.faltan > 0 ? `
                    <form class="marcar-preparado-form-cocina flex items-center gap-3 justify-center" data-op="${item.op_id}">
                      <div class="flex items-center space-x-2">
                        <input type="number" 
                               name="marcar" 
                               value="1" 
                               min="1" 
                               max="${item.faltan}" 
                               class="w-16 px-2 py-1 bg-dark-600/50 border border-dark-500/50 rounded-lg text-white text-center focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        <button type="submit" 
                                class="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-semibold rounded-lg transition-all duration-200 transform hover:scale-105 text-sm">
        ...
        form.addEventListener('submit', function(e) {
          ...
          .then(data => {
            if (data.status === 'ok') {
              Swal.fire({
                title: '¡Excelente!',
                text: 'Producto marcado como preparado',
                icon: 'success',
                timer: 1500,
...
function startAutoRefresh() {
  refreshInterval = setInterval(cargarCocina, 30000);
}
